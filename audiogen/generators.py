...
def tone(frequency=440, min_=-1, max_=1):
    def fixed_tone(frequency):
        ...
        # precompute fixed tone samples # TODO: what about phase glitches at
        # end?
    ...
    def variable_tone(frequency):
        ...
        for f in frequency:
            ...
            # don't reset hard to zero – avoids sudden phase glitches due to
            # rounding error
            if phase > TWO_PI:
                phase -= TWO_PI
...
def synth(freq, angles):
    if isinstance(angles, (int, float)):
        ...
        angles = [0, angles]
    gen = tone(freq)
    loop = list(itertools.islice(gen, (sampler.FRAME_RATE / freq) * (angles[1] / (2.0 * math.pi))))[
