...
def crop(gens, seconds=5, cropper=None):
    ...
    if cropper is None:
        def cropper(gen): return itertools.islice(
            gen, 0, seconds * sampler.FRAME_RATE)
...
def crop_with_fades(gen, seconds, fade_in=0.01, fade_out=0.01):
    ...
    middle = itertools.islice(
        source, 0, total_samples - (fade_in_samples + fade_out_samples))
...
def crop_at_zero_crossing(gen, seconds=5, error=0.1):
    ...
    start = itertools.islice(source, 0, int(
        (seconds - error) * sampler.FRAME_RATE))
    ...
    # find min by sorting buffer samples, first by abs of sample, then by
    # distance from optimal
    best = sorted(
        enumerate(end), key=lambda x: (
            math.fabs(
                x[1]), abs(
                (buffer_length / 2) - x[0])))
    print(best[:10])
...
def channelize(gen, channels):
    ...
    return [
        pick(
            gen_copy, channel) for channel, gen_copy in enumerate(
            itertools.tee(
                gen, channels))]
